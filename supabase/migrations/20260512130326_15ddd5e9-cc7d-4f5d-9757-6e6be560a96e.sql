CREATE OR REPLACE FUNCTION public.get_dashboard_stats()
 RETURNS TABLE(balance numeric, pending_subs numeric, total_subs numeric, expected_income numeric, my_debts numeric, daily_limit numeric, weekly_limit numeric)
 LANGUAGE plpgsql
...
begin
  ...
  -- Daily limit based ONLY on actual balance minus committed expenses.
  -- Expected income is shown separately and NOT added here, otherwise
  -- the user gets a misleadingly high "can spend per day".
  v_free := greatest(0, v_balance - v_pending - v_debts);
